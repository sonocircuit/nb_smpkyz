		...
		if (skGroup.isNil) {
			...
			Routine.new({
				...
				SynthDef(\smpKyzO_mono, {
					...
					// buffer read
					snd = PlayBuf.ar(1, sBuf, rate, gate, startPos, loop: 0, doneAction: 2);
				...
				SynthDef(\smpKyzO_stereo, {
					...
					// buffer read
					snd = PlayBuf.ar(2, sBuf, rate, gate, startPos, loop: 0, doneAction: 2);
					noz = PlayBuf.ar(1, nBuf, startPos: IRand(0, 6 * 48000), loop: 1);
					noz = noz * Amplitude.kr(snd, 0.1, 0.6, noiseAmp);
					snd = (snd + noz);
		...
		StartUp.add {
			...
			OSCFunc.new({ |msg|
				...
				if (skBuffers[buf].notNil) {
					var def = if (skBuffers[buf].numChannels > 1) {\smpKyzL_stereo} {\smpKyzL_mono};
			...
			OSCFunc.new({ |msg|
				...
				var syn;
				if (skBuffers[buf].notNil) {
					var def = if (skBuffers[buf].numChannels > 1) {\smpKyzO_stereo} {\smpKyzO_mono};
					if (skVoices[vox].notNil) { skVoices[vox].set(\gate, -1.05) };
					syn = Synth.new(def,
						[
							\sBuf, skBuffers[buf],
